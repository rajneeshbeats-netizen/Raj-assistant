...
# Increase command-line length limit for Windows with MSYS or Cygwin
if [ -n "$CYGWIN" ] || [ -n "$MSYS" ] || [ -n "$MINGW" ] ; then
    case "$CYGWIN" in
        *CYGWIN*) cygwin=true ;;
    esac
    case "$MSYS" in
...
if [ ! -r "$WRAPPER_JAR" ] ; then
    WRAPPER_JAR="$APP_HOME/gradle/wrapper/gradle-wrapper.properties"
fi
